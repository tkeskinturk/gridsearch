...
2) Calculates the distribution of response trajectories in both the observed and simulated data by estimating slope coefficients at the individual level;

3) Compares the simulated trajectories to the observed trajectories by calculating a Kolmogorov-Smirnov distance between the distribution of simulated slope coefficients and the distribution of observed slope coefficients;

4) Replicates this analysis $N$ times by drawing samples from the specified priors; 
...
```{r}
gridPlot(grid, plot = "conditional", percent = 2)
```
